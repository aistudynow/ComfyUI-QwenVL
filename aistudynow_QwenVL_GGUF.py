...
SYSTEM_PROMPTS_PATH = NODE_DIR / "aistudynow_System_Prompts.json"
...
def _close_llama_object(obj, label: str):
    if obj is None:
        return

    # llama-cpp-python newer builds expose close(); older builds rely on refcount/gc.
    close_fn = getattr(obj, "close", None)
    if callable(close_fn):
        try:
            close_fn()
        except Exception as exc:
            print(f"[QwenVL] Warning: failed to close {label}: {exc}")

    exit_stack = getattr(obj, "_exit_stack", None)
    if exit_stack is not None:
        close_stack = getattr(exit_stack, "close", None)
        if callable(close_stack):
            try:
                close_stack()
            except Exception as exc:
                print(f"[QwenVL] Warning: failed to close {label} exit stack: {exc}")

    for attr in ("_ctx", "_model", "_batch", "_sampler", "_chat_handler"):
...
class QwenVLGGUFBase:
    ...
    def clear(self):
        ...
        _close_llama_object(chat_handler, "GGUF chat handler")
        _close_llama_object(llm, "GGUF model")
...
class aistudynow_QwenVL_GGUF(QwenVLGGUFBase):
    @classmethod
    def INPUT_TYPES(cls):
...
class aistudynow_QwenVL_GGUF_Advanced(QwenVLGGUFBase):
    @classmethod
    def INPUT_TYPES(cls):
        all_models = GGUF_VL_CATALOG.get("models") or {}
        if not isinstance(all_models, dict):
...
NODE_CLASS_MAPPINGS = {
    "aistudynow_QwenVL_GGUF": aistudynow_QwenVL_GGUF,
    "aistudynow_QwenVL_GGUF_Advanced": aistudynow_QwenVL_GGUF_Advanced,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "aistudynow_QwenVL_GGUF": "QwenVL (GGUF)",
    "aistudynow_QwenVL_GGUF_Advanced": "QwenVL Advanced (GGUF)",
}
